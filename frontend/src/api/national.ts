...
export async function updateNetworkFacilityStatus(facilityId: string, status: "APPLICATION" | "ACTIVE" | "SUSPENDED" | "INACTIVE"): Promise<NetworkFacility> { return request(`/api/v1/facilities/network/${facilityId}/status`, { method: "PATCH", body: JSON.stringify({ status }) }); }
