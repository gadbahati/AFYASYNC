...
export function PatientsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    api.listPatients(50, 0).then((data) => { if (!cancelled) { setItems(data.items); setTotal(data.total); } })
      .catch((err) => { if (!cancelled) setError(err instanceof ApiError ? err.code : "PATIENTS_LOAD_FAILED"); })
