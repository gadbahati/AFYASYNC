...
export function NationalBenefitConfigurationPage() {
  ...
  async function loadPayers() {
    const data = await getNetworkPayers({ status: "ACTIVE" });
    setPayers(data);
  ...
  async function loadPlans(id: string) {
    ...
    const data = await getNetworkPayerPlans(id, { status: "ACTIVE" });
    setPlans(data);
    setPlanId("");
  }

  async function loadRules() {
    const data = await getNetworkBenefitRules({ payerId: payerId || undefined, payerPlanId: planId || undefined, status: statusFilter || undefined });
    setRules(data);
  }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await createNetworkBenefitRule({
        payer_id: payerId,
        payer_plan_id: planId || null,
        service_code: serviceCode.trim() || null,
        service_type: serviceType.trim() || null,
        payer_percent: Number(payerPercent),
        fixed_patient_copay: Number(copay),
        max_covered_amount: maxCovered ? Number(maxCovered) : null,
        effective_from: effectiveFrom || null,
        effective_to: effectiveTo || null,
      });
      setMessage("Benefit rule created.");
      setServiceCode(""); setServiceType(""); setMaxCovered(""); setEffectiveFrom(""); setEffectiveTo("");
      await loadRules();
  ...
  function payerName(id: string) { return payers.find((payer) => payer.id === id)?.code ?? id.slice(0, 8); }
  function planName(id: string | null) { return id ? plans.find((plan) => plan.id === id)?.code ?? id.slice(0, 8) : "All plans"; }
  ...
  return <div className="page-stack">
    ...
    {error && <div className="error-banner">{error === "PERMISSION_DENIED" ? "National benefit configuration permission required." : error}</div>}
    ...
    <section className="card"><div className="section-heading"><div><h2>Create benefit rule</h2><p className="muted">Rules require an active payer and, when supplied, an active plan belonging to that payer.</p></div></div>
      <form className="form-grid" onSubmit={submit}>
        ...
        <label>Service code<input value={serviceCode} onChange={(e) => setServiceCode(e.target.value)} placeholder="e.g. LAB-CBC" /></label>
        <label>Service type<input value={serviceType} onChange={(e) => setServiceType(e.target.value)} placeholder="e.g. LABORATORY" /></label>
        <label>Payer coverage %<input type="number" min="0" max="100" step="0.01" value={payerPercent} onChange={(e) => setPayerPercent(e.target.value)} required /></label>
        <label>Patient copay<input type="number" min="0" step="0.01" value={copay} onChange={(e) => setCopay(e.target.value)} required /></label>
        <label>Maximum covered amount<input type="number" min="0" step="0.01" value={maxCovered} onChange={(e) => setMaxCovered(e.target.value)} placeholder="Optional" /></label>
