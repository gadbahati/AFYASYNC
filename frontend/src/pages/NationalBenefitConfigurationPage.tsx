...
export function NationalBenefitConfigurationPage() {
  ...
  const [payerId, setPayerId] = useState(""); const [planId, setPlanId] = useState(""); const [serviceCode, setServiceCode] = useState(""); const [serviceType, setServiceType] = useState("");
  const [payerPercent, setPayerPercent] = useState("100"); const [copay, setCopay] = useState("0"); const [maxCovered, setMaxCovered] = useState(""); const [effectiveFrom, setEffectiveFrom] = useState(""); const [effectiveTo, setEffectiveTo] = useState("");
  ...
  return <div className="page-stack"><div className="page-header"><div><span className="eyebrow">National financing</span><h1>Benefit configuration</h1><p className="muted">Define real payer and plan rules used by AfyaSync responsibility calculations.</p></div><button type="button" className="secondary" onClick={() => void refresh()} disabled={loading}>Refresh</button></div>
    ...
    <section className="card"><div className="section-heading"><div><h2>Create benefit rule</h2><p className="muted">Rules require an active payer and, when supplied, an active plan belonging to that payer.</p></div></div><form className="form-grid" onSubmit={submit}>
      <label>Payer<select value={payerId} onChange={(e) => setPayerId(e.target.value)} required><option value="">Select payer</option>{payers.map((payer) => <option key={payer.id} value={payer.id}>{payer.code} — {payer.name}</option>)}</select></label><label>Plan<select value={planId} onChange={(e) => setPlanId(e.target.value)}><option value="">All plans</option>{plans.map((plan) => <option key={plan.id} value={plan.id}>{plan.code} — {plan.name}</option>)}</select></label>
      <label>Service code<input value={serviceCode} onChange={(e) => setServiceCode(e.target.value)} /></label><label>Service type<input value={serviceType} onChange={(e) => setServiceType(e.target.value)} /></label><label>Payer coverage %<input type="number" min="0" max="100" step="0.01" value={payerPercent} onChange={(e) => setPayerPercent(e.target.value)} required /></label><label>Patient copay<input type="number" min="0" step="0.01" value={copay} onChange={(e) => setCopay(e.target.value)} required /></label><label>Maximum covered amount<input type="number" min="0" step="0.01" value={maxCovered} onChange={(e) => setMaxCovered(e.target.value)} /></label><label>Effective from<input type="date" value={effectiveFrom} onChange={(e) => setEffectiveFrom(e.target.value)} /></label><label>Effective to<input type="date" value={effectiveTo} onChange={(e) => setEffectiveTo(e.target.value)} /></label><div className="form-actions"><button type="submit" disabled={!payerId}>Create rule</button></div>
    </form></section>
    <section className="card"><div className="section-heading"><div><h2>Configured rules</h2><p className="muted">Only persisted payer benefit rules appear here.</p></div><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">All statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></div>{loading ? <div className="empty-state">Loading benefit configuration…</div> : rules.length === 0 ? <div className="empty-state">No benefit rules match the current filters.</div> : <div className="table-wrap"><table><thead><tr><th>Payer</th><th>Plan</th><th>Scope</th><th>Payer %</th><th>Copay</th><th>Maximum</th><th>Effective</th><th>Status</th><th /></tr></thead><tbody>{rules.map((rule) => <tr key={rule.id}><td>{payerName(rule.payer_id)}</td><td>{planName(rule.payer_plan_id)}</td><td>{rule.service_code || rule.service_type}</td><td>{rule.payer_percent}%</td><td>{rule.fixed_patient_copay}</td><td>{rule.max_covered_amount ?? "—"}</td><td>{rule.effective_from || "Any"}{rule.effective_to ? ` → ${rule.effective_to}` : ""}</td><td><span className={`status-badge status-${rule.status.toLowerCase()}`}>{rule.status}</span></td><td><button type="button" className="linkish" onClick={() => void changeStatus(rule)}>{rule.status === "ACTIVE" ? "Deactivate" : "Activate"}</button></td></tr>)}</tbody></table></div>}</section>
  </div>;
}
