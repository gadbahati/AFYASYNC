...
export function PatientSafetyPage() {
  ...
  const load = () => {
    ...
    setLoading(true);
    setError(null);
    api.listPatientAllergies(patientId, true).then(setItems).catch((e: unknown) => setError(e instanceof ApiError ? e.code : "ALLERGIES_LOAD_FAILED")).finally(() => setLoading(false));
  };
