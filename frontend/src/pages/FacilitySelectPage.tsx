...
export function FacilitySelectPage() {
  ...
  async function loadDirectory(search: string, showSpinner = true) {
    ...
    try {
      setFacilities((await api.facilityDirectory(search)) as DirectoryFacility[]);
      setPage(1);
      setError(null);
    } catch (err) {
  ...
  const visibleFacilities = useMemo(
    () => facilities.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE),
    [facilities, page],
  );
    ...
    <main className="auth-page" aria-label="AfyaSync national facility directory" style={{ padding: "28px 16px", alignItems: "flex-start" }}>
      <section className="card" style={{ width: "min(1500px, 100%)", padding: 28, overflow: "hidden" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 20, flexWrap: "wrap", alignItems: "flex-end", marginBottom: 20 }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>
              Search and select the facility where you are working. Facilities are sourced from the national KMHFR registry.
            </p>
          </div>
        ...
        <form onSubmit={searchFacilities} style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 22 }}>
          <input
            ...
            placeholder="Search facility name, MFL code, county, sub-county or facility type…"
            aria-label="Search facilities"
            autoComplete="off"
            style={{ flex: "1 1 520px", minWidth: 0, height: 48, padding: "0 15px", border: "1px solid #cbd5e1", borderRadius: 10, fontSize: 15 }}
          />
          <button type="submit" disabled={loading} style={{ height: 48, padding: "0 26px", border: 0, borderRadius: 10, fontWeight: 700 }}>
            {loading ? "Searching…" : "Search"}
          </button>
          {submittedQuery && (
            <button type="button" onClick={() => { setQuery(""); setSubmittedQuery(""); void loadDirectory(""); }} style={{ height: 48 }}>
              Clear
            </button>
          )}
        </form>
        ...
        {loading ? (
          ...
          <div className="card" style={{ padding: 28, textAlign: "center" }}>
            ...
            <p className="muted" style={{ marginBottom: 0 }}>Try the official facility name, MFL code, county, sub-county or facility type.</p>
          </div>
        ) : (
          <>
            <div
            ...
            >
              {visibleFacilities.map((facility) => (
                <article
                  ...
                  onClick={() => void choose(facility)}
                  onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void choose(facility); } }}
                  role="button"
                  tabIndex={0}
                  aria-label={`Open ${facility.facility_name}`}
                  style={{
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, whiteSpace: "nowrap", padding: "5px 8px", borderRadius: 999, background: "#f1f5f9" }}>
                      {facility.operation_status || "ACTIVE"}
                    </span>
                  </div>
                  ...
                  <button type="button" onClick={(event) => { event.stopPropagation(); void choose(facility); }} disabled={Boolean(selecting)} style={{ marginTop: 16, width: "100%", minHeight: 40 }}>
                    {selecting === facility.facility_id ? "Opening…" : "Open facility"}
                  </button>
                </article>
              ))}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginTop: 20 }}>
              <span className="muted">Showing {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, facilities.length)} of {facilities.length}</span>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <button type="button" disabled={page === 1} onClick={() => setPage((p) => p - 1)}>Previous</button>
                <strong style={{ padding: "8px 6px" }}>Page {page} of {pageCount}</strong>
      ...
      <style>{`
        @media (max-width: 1050px) {
          main[aria-label="AfyaSync national facility directory"] section > div:nth-of-type(3) { grid-template-columns: repeat(2, minmax(0, 1fr)) !important; }
        }
        @media (max-width: 680px) {
          main[aria-label="AfyaSync national facility directory"] section > div:nth-of-type(3) { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </main>
  );
