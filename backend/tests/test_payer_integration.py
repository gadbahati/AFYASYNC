...
def test_payer_callback_updates_integration_transaction_and_claim() -> None:
    ...
    transaction = SimpleNamespace(
        ...
        entity_id=claim_id,
        direction="OUTBOUND",
        request_reference=claim.claim_id,
        status="PENDING",
...
def test_duplicate_payer_callback_is_rejected_before_claim_mutation() -> None:
    ...
    transaction = SimpleNamespace(id=uuid4(), integration_id=integration_id, entity_type="CLAIM", entity_id=claim_id, direction="OUTBOUND", request_reference=claim.claim_id)
...
def test_worker_builds_claim_payload_before_adapter_send() -> None:
    ...
    adapter.send.return_value = SimpleNamespace(
        status="RETRYING",
        response_code="ADAPTER_UNAVAILABLE",
        external_reference=None,
