...
def test_payer_callback_updates_integration_transaction_and_claim() -> None:
    ...
    claim = SimpleNamespace(id=claim_id, claim_id="CLM-CALLBACK", invoice_id=uuid4(), patient_id=uuid4(), payer_id=payer.id, status="SUBMITTED", claim_amount=Decimal("125.00"))
    invoice = SimpleNamespace(id=claim.invoice_id, facility_id=facility_id)
    transaction = SimpleNamespace(id=uuid4(), integration_id=integration_id, entity_type="CLAIM", entity_id=claim_id, direction="OUTBOUND", request_reference=claim.claim_id, status="PENDING", external_reference=None, response_code=None, response_data={}, attempt_count=0, last_attempt_at=None)
    db = MagicMock()
...
def test_worker_builds_claim_payload_before_adapter_send() -> None:
    ...
    db = MagicMock()
    db.scalar.side_effect = [transaction, transaction]
...
def test_callback_signature_accepts_valid_signed_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    secret = "test-callback-secret"
    monkeypatch.setenv("TEST_CALLBACK_SECRET", secret)
    ...
    integration = SimpleNamespace(configuration={"callback_secret_env": "TEST_CALLBACK_SECRET"})

    verify_callback_signature(integration, timestamp, f"sha256={digest}", payload)


def test_callback_signature_rejects_invalid_signature(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TEST_CALLBACK_SECRET", "test-callback-secret")
    integration = SimpleNamespace(configuration={"callback_secret_env": "TEST_CALLBACK_SECRET"})
...
def test_callback_signature_rejects_expired_timestamp(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TEST_CALLBACK_SECRET", "test-callback-secret")
    integration = SimpleNamespace(configuration={"callback_secret_env": "TEST_CALLBACK_SECRET"})
