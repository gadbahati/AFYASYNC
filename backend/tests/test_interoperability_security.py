...
def test_afya_id_normalization_enforces_platform_format():
    ...
    for value in ("AF-42", "AF00000042", "AF-0000000X", ""):
        try:
            normalize_afya_id(value)
        except ValueError as exc:
            assert str(exc) == "INVALID_AFYA_ID"
        else:
            raise AssertionError(f"invalid Afya ID accepted: {value}")
...
def test_auth_rate_limiter_blocks_after_limit():
    ...
    try:
        ...
        assert exc.detail["code"] == "AUTH_RATE_LIMITED"
        assert exc.headers["Retry-After"]
    else:
        raise AssertionError("rate limiter did not block the third attempt")

def test_auth_rate_limiter_ignores_non_auth_paths():
