...
def create_vitals(encounter_id: UUID, payload: VitalCreate, user: User = Depends(get_current_user), token: dict = Depends(get_token_payload), db: Session = Depends(get_db)) -> VitalResponse:
...
def save_consultation(encounter_id: UUID, payload: ConsultationCreate, user: User = Depends(get_current_user), token: dict = Depends(get_token_payload), db: Session = Depends(get_db)) -> ConsultationResponse:
    facility_id = _context(token)
    encounter = _encounter(db, encounter_id, facility_id)
    staff = _staff(db, user, facility_id)
    try:
...
def create_diagnosis(encounter_id: UUID, payload: DiagnosisCreate, user: User = Depends(get_current_user), token: dict = Depends(get_token_payload), db: Session = Depends(get_db)) -> DiagnosisResponse:
    facility_id = _context(token)
    encounter = _encounter(db, encounter_id, facility_id)
    staff = _staff(db, user, facility_id)
    try:
