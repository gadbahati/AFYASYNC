...
from app.clinical.schemas import ClinicalTimelineSummary, ConsultationCreate, ConsultationResponse, DiagnosisCreate, DiagnosisResponse, VitalCreate, VitalResponse
from app.clinical.service import add_diagnosis, create_or_update_consultation, get_encounter_clinical_summary, record_vitals
...
def get_clinical_timeline(encounter_id: UUID, user: User = Depends(require_permission("clinical.record.read")), facility_id: UUID = Depends(get_facility_context), db: Session = Depends(get_db)) -> ClinicalTimelineSummary:
    try:
        ...
        # Materialize the Pydantic response before the audit commit. SQLAlchemy
        # expires ORM instances on commit; returning them afterwards can trigger
        # lazy-load/serialization failures in production.
