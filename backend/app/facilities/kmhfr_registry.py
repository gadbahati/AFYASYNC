...
_state: dict[str, int | bool | str] = {"running": False, "pages": 0, "seen": 0, "changed": 0, "message": "not_started", "source": "none"}
...
def _upsert(db, item: dict) -> bool:
    ...
    existing = db.scalar(select(Facility).where(Facility.registration_number == (code or external)).limit(1)) if (code or external) else None
    if existing is None:
        existing = db.scalar(select(Facility).where(func.lower(Facility.name) == name.lower(), func.lower(func.coalesce(Facility.county, "")) == (county or "").lower()).limit(1))
    if existing is None:
        db.add(Facility(facility_id=_facility_id(key), name=name, facility_type=kind, registration_number=code or external, county=county, sub_county=sub_county, status="ACTIVE" if _active(item) else "INACTIVE"))
        return True
    changed = False
    values = {"name": name, "facility_type": kind, "county": county, "sub_county": sub_county, "status": "ACTIVE" if _active(item) else "INACTIVE"}
    if code or external:
        values["registration_number"] = code or external
...
def sync_all(*, force: bool = False) -> dict[str, int | bool | str]:
    ...
    try:
        if not SNAPSHOT.exists():
            raise RuntimeError("KMHFR_SNAPSHOT_MISSING")
        if not bool(db.execute(text("SELECT pg_try_advisory_lock(hashtextextended(:key, 0))"), {"key": LOCK_KEY}).scalar()):
    ...
    def worker():
        ...
        while True:
            try:
                ...
                if count >= 10000:
                    logger.info("KMHFR_REGISTRY_READY active_facilities=%s", count)
                    return
                if not _running:
                    logger.info("KMHFR_REGISTRY_RETRY active_facilities=%s", count)
                    start_sync()
            except Exception:
