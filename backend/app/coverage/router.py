...
from app.auth.dependencies import get_current_user, get_facility_context, require_patient_identity, require_permission
...
from app.coverage.oop_schemas import OopEstimateRequest, OopEstimateResponse
from app.coverage.oop_service import estimate_out_of_pocket
...
def adjudicate(
    ...
    try:
        return adjudicate_benefit(
            db, payload=payload, facility_id=facility_id, actor_user_id=user.id
        )
    except ValueError as err:
        code = str(err)
        mapping = {
...
@router.post("/oop-estimate", response_model=OopEstimateResponse)
def facility_oop_estimate(
    payload: OopEstimateRequest,
    db: Session = Depends(get_db),
    facility_id: UUID = Depends(get_facility_context),
    user: User = Depends(require_permission(COVERAGE_READ)),
) -> OopEstimateResponse:
    """Multi-payer truth table + patient out-of-pocket estimate (facility)."""
    try:
        result = estimate_out_of_pocket(
            db,
            payload=payload,
            facility_id=facility_id,
            actor_user_id=user.id,
            require_facility_enrollment=True,
        )
        db.commit()
        return result
    except ValueError as err:
        ...
        mapping = {
            "PATIENT_NOT_IN_FACILITY": (404, "Patient is not enrolled at this facility."),
            "FACILITY_REQUIRED": (400, "Facility context required."),
            "INVALID_GROSS": (400, "Invalid line totals."),
            "SERVICE_SCOPE_REQUIRED": (400, "Each line needs service_code or service_type."),
        }
        http_status, message = mapping.get(code, (400, code))
        raise HTTPException(
            status_code=http_status, detail={"code": code, "message": message}
...
@router.post("/portal/oop-estimate", response_model=OopEstimateResponse)
def portal_oop_estimate(
    payload: OopEstimateRequest,
    db: Session = Depends(get_db),
    user: User = Depends(require_patient_identity),
) -> OopEstimateResponse:
    """Patient self-service OOP estimate across own coverages."""
    try:
        person_id = require_patient_person_id(user.person_id)
    except PortalError as err:
        raise HTTPException(status_code=403, detail=str(err)) from err
    if payload.person_id != person_id:
        raise HTTPException(status_code=403, detail="COVERAGE_ACCESS_DENIED")
    try:
        result = estimate_out_of_pocket(
            db,
            payload=payload,
            facility_id=None,
            actor_user_id=user.id,
            require_facility_enrollment=False,
        )
        db.commit()
        return result
    except ValueError as err:
        code = str(err)
        raise HTTPException(
            status_code=400, detail={"code": code, "message": code}
        ) from err
