...
def process_preauthorization_callback(
    ...
    *,
    ...
    authorization = db.scalar(
        ...
        .where(PreAuthorization.id == authorization_id)
        .with_for_update()
    )
    ...
    if transaction.status == "SUCCEEDED" and transaction.external_reference:
        if transaction.external_reference == external_reference:
            return authorization
        raise PreAuthorizationCallbackError("DUPLICATE_PREAUTH_RESPONSE")

    transaction.status = "PENDING" if status == "PENDING" else "SUCCEEDED"
