...
from app.integrations.schemas import IntegrationCreate, IntegrationOut, IntegrationStatusUpdate, PayerCallbackCreate, PaymentCallbackCreate, TransactionCreate, TransactionMonitorOut, TransactionOut
from app.integrations.service import IntegrationError, create_integration, list_integration_transactions, list_integrations, queue_transaction, update_integration_status, verify_callback_signature
...
def _error(exc: IntegrationError | ClaimsError | BillingError | PreAuthorizationCallbackError) -> HTTPException:
    mapping = {
        ...
        "INTEGRATION_STATUS_REASON_REQUIRED": 400,
    }
...
def update_status(integration_id: UUID, payload: IntegrationStatusUpdate, db: Session = Depends(get_db), facility_id: UUID = Depends(get_facility_context), user: User = Depends(require_permission(INTEGRATIONS_WRITE))):
    try:
        return update_integration_status(db, integration_id, facility_id, payload.status, payload.reason, user.id)
    except IntegrationError as exc:
        db.rollback(); raise _error(exc) from exc
