...
def _facility(token: dict) -> UUID:
    ...
    if not raw: raise HTTPException(status_code=403, detail="FACILITY_CONTEXT_REQUIRED")
    try: return UUID(raw)
    except (ValueError, TypeError) as exc: raise HTTPException(status_code=403, detail="INVALID_FACILITY_CONTEXT") from exc
...
def create_medication(payload: MedicationCreate, db: Session = Depends(get_db), user: User = Depends(require_permission(PHARMACY_CREATE_MEDICATION)), token: dict = Depends(get_token_payload)) -> Medication:
    facility_id = _facility(token); _staff(db, user, facility_id)
    if db.scalar(select(Medication).where(Medication.code == payload.code)): raise HTTPException(status_code=409, detail="MEDICATION_CODE_EXISTS")
    medication = Medication(**payload.model_dump()); db.add(medication); db.flush()
    ...
    db.commit(); db.refresh(medication); return medication
...
def create_prescription(payload: PrescriptionCreate, db: Session = Depends(get_db), user: User = Depends(require_permission(PHARMACY_CREATE_PRESCRIPTION)), token: dict = Depends(get_token_payload)) -> Prescription:
    facility_id = _facility(token); staff = _staff(db, user, facility_id); encounter = db.get(Encounter, payload.encounter_id)
...
def receive_inventory(payload: InventoryReceive, db: Session = Depends(get_db), user: User = Depends(require_permission(PHARMACY_RECEIVE_INVENTORY)), token: dict = Depends(get_token_payload)) -> InventoryItem:
    facility_id = _facility(token); staff = _staff(db, user, facility_id)
    if payload.facility_id != facility_id: raise HTTPException(status_code=403, detail="FACILITY_ACCESS_DENIED")
    ...
    if payload.expiry_date < date.today(): raise HTTPException(status_code=400, detail="EXPIRED_BATCH")
    ...
    if item is None: item = InventoryItem(facility_id=facility_id, medication_id=payload.medication_id, current_quantity=0, minimum_quantity=0); db.add(item); db.flush()
    ...
    if batch is None: batch = InventoryBatch(inventory_item_id=item.id, batch_number=payload.batch_number, expiry_date=payload.expiry_date, quantity=payload.quantity, purchase_price=payload.purchase_price, selling_price=payload.selling_price); db.add(batch)
    else:
        if batch.expiry_date != payload.expiry_date: raise HTTPException(status_code=409, detail="BATCH_EXPIRY_MISMATCH")
        batch.quantity += payload.quantity; batch.purchase_price = payload.purchase_price; batch.selling_price = payload.selling_price
    item.current_quantity += payload.quantity; db.flush(); db.add(StockMovement(inventory_item_id=item.id, batch_id=batch.id, movement_type="RECEIVE", quantity=payload.quantity, reference_type="INVENTORY_RECEIPT", performed_by=staff.id)); db.flush()
    record_audit(db, action="PHARMACY_INVENTORY_RECEIVED", resource_type="INVENTORY_ITEM", resource_id=str(item.id), result="SUCCESS", user_id=user.id, facility_id=facility_id, metadata={"batch_number": payload.batch_number, "quantity": payload.quantity}, commit=False); db.commit(); db.refresh(item); return item
...
def dispense(prescription_id: UUID, payload: DispenseRequest, db: Session = Depends(get_db), user: User = Depends(require_permission(PHARMACY_DISPENSE)), token: dict = Depends(get_token_payload)) -> DispenseResponse:
    facility_id = _facility(token); staff = _staff(db, user, facility_id); prescription = db.get(Prescription, prescription_id)
    if prescription is None: raise HTTPException(status_code=404, detail="PRESCRIPTION_NOT_FOUND")
    encounter = db.get(Encounter, prescription.encounter_id)
    if encounter is None: raise HTTPException(status_code=404, detail="ENCOUNTER_NOT_FOUND")
    if encounter.facility_id != facility_id: raise HTTPException(status_code=403, detail="FACILITY_ACCESS_DENIED")
    try: movements, charges_created = dispense_prescription(db, prescription_id, staff.id, actor_user_id=user.id, billing_items=[item.model_dump() for item in payload.billing_items])
    except PharmacyError as exc: raise _error(exc) from exc
