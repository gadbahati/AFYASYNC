...
def get(encounter_id: UUID, _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
def close(encounter_id: UUID, _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
