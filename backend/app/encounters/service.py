...
def create_encounter(db: Session, data: dict, created_by: UUID, *, actor_user_id: UUID | None = None, commit: bool = True) -> Encounter:
    ...
    db.flush()
    if actor_user_id:
        record_audit(db, action="ENCOUNTER_CREATED", resource_type="ENCOUNTER", resource_id=str(encounter.id), result="SUCCESS", user_id=actor_user_id, facility_id=encounter.facility_id, patient_id=encounter.patient_id, metadata={"encounter_id": encounter.encounter_id, "encounter_type": encounter.encounter_type, "coverage_mode": encounter.coverage_mode, "coverage_id": str(encounter.coverage_id) if encounter.coverage_id else None}, commit=False)
    if commit:
        db.commit()
