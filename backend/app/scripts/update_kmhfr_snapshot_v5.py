...
def fetch_page(page: int) -> dict | list:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": "Mozilla/5.0 AfyaSync-KMHFR/5.0", "Connection": "close"})
...
def main() -> None:
    print(f"KMHFR v5: starting official national import source={API}", flush=True)
    first = fetch_page(1)
    ...
    print(f"KMHFR v5: API count={count} pages={total_pages} workers={WORKERS}", flush=True)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=WORKERS) as pool:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if completed % 20 == 0 or completed == total_pages:
                print(f"KMHFR v5: progress page={completed}/{total_pages} records={len(records)}", flush=True)
