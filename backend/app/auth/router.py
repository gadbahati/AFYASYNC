...
def login(payload: LoginRequest, request: Request, db: Session = Depends(get_db), _: None = Depends(enforce_auth_rate_limit)) -> TokenResponse | FacilitySelectionRequired:
    ...
    if len(staff) != 1:
        ...
        return FacilitySelectionRequired(access_token=issue_access_token(user, None), facilities=[FacilityOption(facility_id=row[0], facility_name=row[1]) for row in rows])
