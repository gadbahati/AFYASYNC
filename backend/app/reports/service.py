...
def _window(start_date: date, end_date: date) -> tuple[datetime, datetime]:
    if end_date < start_date: raise ValueError("INVALID_REPORT_DATE_RANGE")
    return datetime.combine(start_date, time.min, tzinfo=timezone.utc), datetime.combine(end_date, time.max, tzinfo=timezone.utc)

def _money(value: object) -> Decimal: return Decimal(str(value or 0)).quantize(Decimal("0.01"))
...
def build_facility_report(db: Session, facility_id: UUID, start_date: date, end_date: date, *, actor_user_id: UUID | None = None) -> dict[str, object]:
    ...
    claim_statuses=[{"status":r[0],"count":int(r[1] or 0),"amount":_money(r[2]),"approved_amount":_money(r[3]),"paid_amount":_money(r[4])} for r in status_rows]
    ...
    payer_claims=[{"payer_id":str(r[0]),"payer_name":r[1],"payer_code":r[2],"claims":int(r[3] or 0),"amount":_money(r[4]),"approved_amount":_money(r[5]),"paid_amount":_money(r[6]),"receivable":max(_money(r[5])-_money(r[6]),Decimal("0.00"))} for r in payer_rows]
...
def build_facility_operations_report(db: Session, facility_id: UUID, start_date: date, end_date: date, *, actor_user_id: UUID | None = None) -> dict[str, object]:
    start,end=_window(start_date,end_date); enc_filter=(Encounter.facility_id==facility_id,Encounter.created_at>=start,Encounter.created_at<=end)
    patients=int(db.scalar(select(func.count(PatientFacility.id)).where(PatientFacility.facility_id==facility_id,PatientFacility.created_at>=start,PatientFacility.created_at<=end)) or 0)
    ...
    received=sum((Decimal(str(q or 0)) for t,q in movement_rows if t in {"RECEIVE","RECEIPT"}),Decimal("0.00")); dispensed=sum((Decimal(str(q or 0)) for t,q in movement_rows if t in {"DISPENSE","ISSUE"}),Decimal("0.00"))
    diagnosis_rows=db.execute(select(Diagnosis.diagnosis_name,func.count(Diagnosis.id)).join(Encounter,Encounter.id==Diagnosis.encounter_id).where(*enc_filter).group_by(Diagnosis.diagnosis_name).order_by(func.count(Diagnosis.id).desc(),Diagnosis.diagnosis_name).limit(20)).all(); top_diagnoses=[{"diagnosis":r[0],"count":int(r[1] or 0)} for r in diagnosis_rows]
    daily=[]; current=start_date
    while current<=end_date:
        ds,de=_window(current,current); f=(Encounter.facility_id==facility_id,Encounter.created_at>=ds,Encounter.created_at<=de)
        dp=int(db.scalar(select(func.count(PatientFacility.id)).where(PatientFacility.facility_id==facility_id,PatientFacility.created_at>=ds,PatientFacility.created_at<=de)) or 0); de_count=int(db.scalar(select(func.count(Encounter.id)).where(*f)) or 0); dd=int(db.scalar(select(func.count(Diagnosis.id)).join(Encounter,Encounter.id==Diagnosis.encounter_id).where(*f)) or 0); dr=int(db.scalar(select(func.count(Prescription.id)).join(Encounter,Encounter.id==Prescription.encounter_id).where(*f)) or 0); da=int(db.scalar(select(func.count(Admission.id)).where(Admission.facility_id==facility_id,Admission.created_at>=ds,Admission.created_at<=de)) or 0); df=int(db.scalar(select(func.count(Referral.id)).where(Referral.source_facility_id==facility_id,Referral.created_at>=ds,Referral.created_at<=de)) or 0)
        di=_money(db.scalar(select(func.coalesce(func.sum(Invoice.total_amount),0)).where(Invoice.facility_id==facility_id,Invoice.created_at>=ds,Invoice.created_at<=de,Invoice.status!="VOID"))); dpmt=_money(db.scalar(select(func.coalesce(func.sum(Payment.amount),0)).where(Payment.facility_id==facility_id,Payment.created_at>=ds,Payment.created_at<=de,Payment.status=="CONFIRMED"))); dc=_money(db.scalar(select(func.coalesce(func.sum(Charge.total_amount),0)).where(Charge.facility_id==facility_id,Charge.created_at>=ds,Charge.created_at<=de,Charge.status=="ACTIVE")))
        mv=db.execute(select(StockMovement.movement_type,func.coalesce(func.sum(StockMovement.quantity),0)).join(InventoryItem,InventoryItem.id==StockMovement.inventory_item_id).where(InventoryItem.facility_id==facility_id,StockMovement.created_at>=ds,StockMovement.created_at<=de).group_by(StockMovement.movement_type)).all(); din=sum((Decimal(str(q or 0)) for t,q in mv if t in {"RECEIVE","RECEIPT"}),Decimal("0.00")); dout=sum((Decimal(str(q or 0)) for t,q in mv if t in {"DISPENSE","ISSUE"}),Decimal("0.00"))
        daily.append({"date":current,"patients":dp,"encounters":de_count,"diagnoses":dd,"prescriptions":dr,"admissions":da,"referrals":df,"charges":dc,"invoices":di,"payments":dpmt,"stock_received_quantity":din,"stock_dispensed_quantity":dout}); current+=timedelta(days=1)
