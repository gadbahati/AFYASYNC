...
def build_facility_operations_report(db: Session, facility_id: UUID, start_date: date, end_date: date, *, actor_user_id: UUID | None = None) -> dict[str, object]:
    ...
    received = sum((Decimal(str(q or 0)) for t, q in movement_rows if t == "RECEIVE"), Decimal("0.00"))
    ...
    while current <= end_date:
        ...
        day_received = sum((Decimal(str(q or 0)) for t, q in day_movements if t == "RECEIVE"), Decimal("0.00"))
        day_dispensed = sum((Decimal(str(q or 0)) for t, q in day_movements if t in {"DISPENSE", "ISSUE"}), Decimal("0.00"))
