...
def _error(exc: ValueError) -> HTTPException:
    code = str(exc)
    mapping = {"PATIENT_NOT_FOUND": 404, "FACILITY_NOT_FOUND": 404, "DEPARTMENT_NOT_FOUND": 404, "PROVIDER_NOT_FOUND": 404, "QUEUE_NOT_FOUND": 404, "QUEUE_ENTRY_NOT_FOUND": 404, "ENCOUNTER_NOT_FOUND": 404, "ENCOUNTER_CLOSED": 409, "INVALID_APPOINTMENT": 400, "PATIENT_ALREADY_QUEUED": 409, "INVALID_QUEUE_TRANSITION": 409}
...
def create_queue_endpoint(payload: QueueCreate, _: User = Depends(require_permission("appointments.write")), facility_id: UUID = Depends(get_facility_context), db: Session = Depends(get_db)):
    data = payload.model_dump(); data["facility_id"] = facility_id
...
def list_queues_endpoint(_: User = Depends(require_permission("appointments.read")), facility_id: UUID = Depends(get_facility_context), db: Session = Depends(get_db)):
    return list_queues(db, facility_id)


@router.get("/queues/entries", response_model=list[QueueEntryResponse])
def list_queue_entries_endpoint(queue_id: UUID | None = Query(default=None), _: User = Depends(require_permission("appointments.read")), facility_id: UUID = Depends(get_facility_context), db: Session = Depends(get_db)):
...
def add_queue_entry(payload: QueueEntryCreate, user: User = Depends(require_permission("queue.checkin")), facility_id: UUID = Depends(get_facility_context), db: Session = Depends(get_db)):
    entry_queue = db.scalar(select(Queue).where(Queue.id == payload.queue_id))
    if entry_queue is None or entry_queue.facility_id != facility_id: raise HTTPException(status_code=403, detail="FACILITY_ACCESS_DENIED")
    try: return add_to_queue(db, payload.model_dump(), user.id, actor_user_id=user.id)
    except ValueError as err: raise _error(err) from err
