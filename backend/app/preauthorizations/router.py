...
def _error(exc: PreAuthorizationError) -> HTTPException:
    ...
    mapping = {
        ...
        "PREAUTH_NOT_DECIDABLE": 409, "APPROVED_AMOUNT_EXCEEDS_REQUEST": 400,
        "REJECTED_AMOUNT_MUST_BE_ZERO": 400, "PREAUTHORIZATION_CREATE_FAILED": 409,
    }
...
def check_eligibility(
    ...
    try:
        result = check_coverage_eligibility(db, patient_id=payload.patient_id, coverage_id=payload.coverage_id, service_code=payload.service_code.strip().upper() if payload.service_code else None, service_type=payload.service_type.strip().upper() if payload.service_type else None, as_of=payload.as_of, actor_user_id=user.id, facility_id=facility_id)
