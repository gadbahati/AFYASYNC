...
def get_patient_record_summary(db: Session, patient_id: UUID, facility_id: UUID) -> dict | None:
    ...
    care_plans = list(db.execute(select(CarePlan).where(CarePlan.patient_id == patient_id, CarePlan.facility_id == facility_id).order_by(CarePlan.updated_at.desc())).scalars())
    allergies = list(db.execute(select(Allergy).where(Allergy.patient_id == patient_id, Allergy.facility_id == facility_id).order_by(Allergy.updated_at.desc(), Allergy.created_at.desc())).scalars())
    ...
    return {
        ...
        "allergies": [
            {
                "id": str(item.id), "allergen": item.allergen, "reaction": item.reaction,
                "severity": item.severity, "onset_date": item.onset_date, "notes": item.notes,
                "status": item.status, "created_at": item.created_at, "updated_at": item.updated_at,
            } for item in allergies
        ],
        "care_plans": [{"id": str(p.id), "patient_id": str(p.patient_id), "facility_id": str(p.facility_id), "encounter_id": str(p.encounter_id) if p.encounter_id else None, "created_by": str(p.created_by), "title": p.title, "goals": p.goals, "interventions": p.interventions, "clinical_notes": p.clinical_notes, "target_date": p.target_date, "status": p.status, "completed_at": p.completed_at, "created_at": p.created_at, "updated_at": p.updated_at} for p in care_plans],
