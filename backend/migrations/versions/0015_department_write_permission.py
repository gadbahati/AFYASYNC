"""add facility department management permission

Revision ID: 0015_department_write_permission
...
revision = "0015_department_write_permission"
down_revision = "0014_patient_create_permission"
branch_labels = None
depends_on = None

PERMISSION_CODE = "facilities.department.write"
    ...
    if permission_id is None:
        ...
        bind.execute(
            sa.insert(permissions).values(
                ...
                description="Create facility departments",
            )
        )

    role_names = {"Hospital Administrator", "System Administrator"}
    for role_id, role_name in bind.execute(
        sa.select(roles.c.id, roles.c.name).where(roles.c.name.in_(role_names))
    ).fetchall():
        exists = bind.execute(
