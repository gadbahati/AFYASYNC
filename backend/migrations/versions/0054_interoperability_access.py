"""Add explicit interoperability read permission and merge current national-access heads.

Revision ID: 0054_interoperability_access
Revises: 0053_national_record_locator_access, 0053_sha_eligibility_connector
"""
...
revision = "0054_interoperability_access"
down_revision = ("0053_national_record_locator_access", "0053_sha_eligibility_connector")
branch_labels = None
depends_on = None

PERMISSION_CODE = "interoperability.patient.read"
ROLE_NAME = "Healthcare Interoperability Administrator"
...
def upgrade() -> None:
    ...
    if permission_id is None:
        ...
        bind.execute(sa.insert(permissions).values(
            ...
            description="Read a minimum necessary FHIR-compatible patient resource within an authorised facility",
        ))
    ...
    if role_id is None:
        ...
        bind.execute(sa.insert(roles).values(
            ...
            description="Explicitly privileged access to healthcare interoperability patient reads",
        ))
