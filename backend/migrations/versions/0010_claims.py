...
def upgrade() -> None:
    op.create_table("claims", _uuid(),
        sa.Column("claim_id", sa.String(70), nullable=False),
    ...
    for name, col in [("ix_claims_claim_id", "claim_id"), ("ix_claims_invoice_id", "invoice_id"), ("ix_claims_encounter_id", "encounter_id"), ("ix_claims_patient_id", "patient_id"), ("ix_claims_payer_id", "payer_id"), ("ix_claims_status", "status")]:
        op.create_index(name, "claims", [col], unique=False)
    op.create_table("claim_items", _uuid(),
    ...
    op.create_index("ix_claim_items_claim_id", "claim_items", ["claim_id"], unique=False)
    op.create_table("claim_responses", _uuid(),
        sa.Column("claim_id", postgresql.UUID(as_uuid=True), nullable=False), sa.Column("external_reference", sa.String(150)), sa.Column("status", sa.String(40), nullable=False), sa.Column("response_code", sa.String(80)), sa.Column("response_message", sa.String(500)), sa.Column("received_at", sa.DateTime(timezone=True), server_default=sa.text("now()")),
        sa.ForeignKeyConstraint(["claim_id"], ["claims.id"], ondelete="RESTRICT"), sa.PrimaryKeyConstraint("id"))
    op.create_index("ix_claim_responses_claim_id", "claim_responses", ["claim_id"], unique=False)
    op.create_table("reconciliations", _uuid(),
        sa.Column("claim_id", postgresql.UUID(as_uuid=True), nullable=False), sa.Column("expected_amount", sa.Numeric(14, 2), nullable=False), sa.Column("received_amount", sa.Numeric(14, 2), nullable=False), sa.Column("difference", sa.Numeric(14, 2), nullable=False), sa.Column("status", sa.String(30), nullable=False, server_default="PENDING"), sa.Column("reconciled_by", postgresql.UUID(as_uuid=True)), sa.Column("reconciled_at", sa.DateTime(timezone=True)),
        sa.ForeignKeyConstraint(["claim_id"], ["claims.id"], ondelete="RESTRICT"), sa.ForeignKeyConstraint(["reconciled_by"], ["staff.id"], ondelete="RESTRICT"), sa.PrimaryKeyConstraint("id"), sa.UniqueConstraint("claim_id", name="uq_reconciliation_claim"))
    op.create_index("ix_reconciliations_claim_id", "reconciliations", ["claim_id"], unique=False)
